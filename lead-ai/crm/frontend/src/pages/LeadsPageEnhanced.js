...
const makeDateFilter = (fieldKey) => ({
  ...
  onFilter: (value, record) => {
    ...
    if (!val) return false;
    // parseDate appends 'Z' when no timezone offset is present so Supabase
    // UTC timestamps are treated correctly regardless of the local timezone.
    const d = parseDate(val);
    if (!d) return false;
    const now = dayjs();
    const { mode, date, range } = value;
    if (mode === 'today')  return d.isSame(now, 'day');
    if (mode === 'week')   return d.isAfter(now.subtract(7, 'day').startOf('day'));
    if (mode === 'month')  return d.isAfter(now.subtract(1, 'month').startOf('day'));
    if (mode === 'on'      && date)              return d.isSame(dayjs(date), 'day');
    if (mode === 'before'  && date)              return d.isBefore(dayjs(date).startOf('day'));
    if (mode === 'after'   && date)              return d.isAfter(dayjs(date).endOf('day'));
...
// ── Date column filter → server param translator ──────────────────────────
// Translates an Ant Design column filter value (from makeDateFilter) into
// the backend query params stored in the `cf` object.
//   val        – tableFilters[colKey]  (array from Ant Design, item is an object)
//   fromKey    – e.g. 'created_from'   (used for today/week/month/on/between/after)
//   toKey      – e.g. 'created_to'     (used for today/week/month/on/between/before)
//   beforeKey  – e.g. 'created_before' (exclusive upper-bound, null if unsupported)
//   afterKey   – e.g. 'created_after'  (exclusive lower-bound, null if unsupported)
//   cf         – mutable params object to populate
const applyDateColFilter = (val, fromKey, toKey, beforeKey, afterKey, cf) => {
  if (!val?.[0]) return;
  ...
  if (mode === 'today') {
    ...
    // Exact day: from 00:00 to 23:59 of that day
    cf[fromKey] = dayjs(date).startOf('day').toISOString();
    cf[toKey]   = dayjs(date).endOf('day').toISOString();
  } else if (mode === 'before' && date) {
    // Before a date: records strictly before the start of that day
    if (beforeKey) {
      cf[beforeKey] = dayjs(date).startOf('day').toISOString();
    } else {
      // fallback: use toKey with exclusive upper bound
      cf[toKey] = dayjs(date).subtract(1, 'second').toISOString();
    }
  } else if (mode === 'after' && date) {
    // After a date: records strictly after the end of that day
    if (afterKey) {
      cf[afterKey] = dayjs(date).endOf('day').toISOString();
    } else {
      // fallback: use fromKey with exclusive lower bound
      cf[fromKey] = dayjs(date).add(1, 'day').startOf('day').toISOString();
    }
...
const LeadsPageEnhanced = () => {
  ...
  const [columnFilters, setColumnFilters] = useState(() => {
    try {
      ...
      // Restore date column filters from session
      applyDateColFilter(tf.follow_up_date, 'follow_up_from', 'follow_up_to', null,               null,              cf);
      applyDateColFilter(tf.created_at,     'created_from',   'created_to',   'created_before',   'created_after',   cf);
      applyDateColFilter(tf.updated_at,     'updated_from',   'updated_to',   'updated_before',   'updated_after',   cf);
      return cf;
    } catch { return {}; }
  });
  ...
  const leadQueryParams = React.useMemo(() => {
    ...
    if (advFilters.qualification?.length) params.qualification_in = advFilters.qualification.join(',');
    ...
    // Created date filter (from Advanced Filter drawer)
    if (advFilters.createdDate) {
      applyDateColFilter(
        [advFilters.createdDate], 'created_from', 'created_to', 'created_before', 'created_after', params
      );
    }

    // Updated date filter (from Advanced Filter drawer)
    if (advFilters.updatedDate) {
      applyDateColFilter(
        [advFilters.updatedDate], 'updated_from', 'updated_to', 'updated_before', 'updated_after', params
      );
    }
  ...
  // Every lead that has been submitted more than once (one row per person,
  // kept with its original owner). Sorted newest repeat first by the backend.
  const repeatedLeadsList = useMemo(
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }} size={16}>
          ...
          <div>
            ...
            <Select mode="multiple" style={{ width: '100%', marginTop: 6 }} placeholder="Any qualification" showSearch
              value={advFilters.qualification || []} onChange={v => setAdvFilters(f => ({ ...f, qualification: v }))}>
              {QUALIFICATION_OPTIONS.map(q => <Option key={q} value={q}>{q}</Option>)}
            </Select>
          </div>
          ...
          <div>
            ...
            <Select style={{ width: '100%', marginTop: 6 }} placeholder="Any time" allowClear
              value={advFilters.createdDate?.mode}
              onChange={v => setAdvFilters(f => ({ ...f, createdDate: v ? { mode: v } : null }))}>
              <Option value="today">Today</Option>
            ...
            {advFilters.createdDate?.mode === 'between' && (
              <RangePicker style={{ width: '100%', marginTop: 6 }}
                onChange={r => setAdvFilters(f => ({ ...f, createdDate: { ...f.createdDate, range: r?.map(d => d.format('YYYY-MM-DD')) } }))} />
            )}
            {['on','before','after'].includes(advFilters.createdDate?.mode) && (
              <DatePicker style={{ width: '100%', marginTop: 6 }}
                onChange={d => setAdvFilters(f => ({ ...f, createdDate: { ...f.createdDate, date: d?.format('YYYY-MM-DD') } }))} />
            )}
          </div>

          <div>
            <Text strong>Last Updated Date</Text>
            <Select style={{ width: '100%', marginTop: 6 }} placeholder="Any time" allowClear
              value={advFilters.updatedDate?.mode}
              onChange={v => setAdvFilters(f => ({ ...f, updatedDate: v ? { mode: v } : null }))}>
              <Option value="today">Today</Option>
              <Option value="week">Last 7 days</Option>
              <Option value="month">Last 30 days</Option>
              <Option value="on">On exact date</Option>
            ...
            {advFilters.updatedDate?.mode === 'between' && (
              <RangePicker style={{ width: '100%', marginTop: 6 }}
                onChange={r => setAdvFilters(f => ({ ...f, updatedDate: { ...f.updatedDate, range: r?.map(d => d.format('YYYY-MM-DD')) } }))} />
            )}
            {['on','before','after'].includes(advFilters.updatedDate?.mode) && (
              <DatePicker style={{ width: '100%', marginTop: 6 }}
                onChange={d => setAdvFilters(f => ({ ...f, updatedDate: { ...f.updatedDate, date: d?.format('YYYY-MM-DD') } }))} />
            )}
          </div>
      ...
      >
        {(repeatedData?.total || 0) === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 0', color: '#6b7280' }}>
            ...
            <div style={{ fontSize: 13, marginTop: 4 }}>Nobody has submitted an enquiry more than once yet.</div>
          </div>
        ) : (
          <div>
            <Alert
            ...
            <Collapse
              accordion
              items={repeatedLeadsList.map((lead) => {
                const statusColor = lead.status === 'Enrolled' ? 'green'
                  : lead.status === 'Fresh' ? 'blue'
                  : lead.status === 'Not Interested' ? 'red' : 'default';
                return {
                  key: String(lead.lead_id),
                  extra: (
                    <a onClick={(e) => { e.stopPropagation(); setRepeatDrawerVisible(false); navigate(`/leads/${lead.lead_id}`); }}>
                      Open lead
                    </a>
                  ),
                  label: (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontWeight: 600, fontSize: 13 }}>{lead.full_name}</span>
                      <Tag color="volcano" style={{ fontSize: 11 }}>Repeated ×{lead.submission_count || 2}</Tag>
                      <Tag color={statusColor} style={{ fontSize: 11 }}>{lead.status}</Tag>
                      <span style={{ fontSize: 11, color: '#6b7280' }}>
                        <UserOutlined /> {lead.assigned_to || 'Unassigned'}
                      </span>
                      {lead.last_submission_at && (
                        <span style={{ fontSize: 11, color: '#9ca3af' }}>
                          last {dayjs(lead.last_submission_at).fromNow()}
                        </span>
                      )}
                    </div>
                  ),
                  children: (
                    <>
                      <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 10, lineHeight: 1.9 }}>
                        ...
                        {lead.email && <span style={{ marginRight: 12 }}>✉ {lead.email}</span>}
                        {lead.course_interested && <span style={{ marginRight: 12 }}>📚 {lead.course_interested}</span>}
                        <br />
                        First: <b>{lead.first_submission_at ? dayjs(lead.first_submission_at).format('D MMM YYYY') : '—'}</b>
                        {'  ·  '}
                        Last: <b>{lead.last_submission_at ? dayjs(lead.last_submission_at).format('D MMM YYYY') : '—'}</b>
                        {lead.last_submission_campaign && <span>{'  ·  '}via {lead.last_submission_campaign}</span>}
                        {Array.isArray(lead.repeat_channels) && lead.repeat_channels.length > 0 && (
                          <span>{'  ·  '}channels: {lead.repeat_channels.join(', ')}</span>
                        )}
                      </div>
                      <SubmissionTimeline leadId={lead.lead_id} />
                    </>
                  ),
                };
              })}
