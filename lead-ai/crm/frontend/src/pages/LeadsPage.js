...
import { hasAnyPermission, PERMISSIONS } from '../config/rbac';
...
const LeadsPage = () => {
  ...
  const [waInboxVisible, setWaInboxVisible] = useState(false);

  const canAccessWhatsApp = hasAnyPermission(user?.role, [
    PERMISSIONS.VIEW_OWN_WHATSAPP,
    PERMISSIONS.VIEW_ALL_WHATSAPP,
  ]);
  const [filters, setFilters] = useState(() => {
    try {
      const saved = sessionStorage.getItem('leadsPage_filters');
      return saved ? JSON.parse(saved) : {};
    } catch { return {}; }
  });
  const [filteredInfo, setFilteredInfo] = useState(() => {
    try {
      const saved = sessionStorage.getItem('leadsPage_filteredInfo');
      return saved ? JSON.parse(saved) : {};
    } catch { return {}; }
  });
  ...
  const handleFilter = (key, value) => {
    setFilters(prev => {
      ...
      try { sessionStorage.setItem('leadsPage_filters', JSON.stringify(next)); } catch {}
      return next;
    });
  };

  const handleTableChange = (_pagination, columnFilters) => {
    setFilteredInfo(columnFilters);
    try { sessionStorage.setItem('leadsPage_filteredInfo', JSON.stringify(columnFilters)); } catch {}
  };
  ...
  const clearFilters = () => {
    ...
    setFilteredInfo({});
    try {
      sessionStorage.removeItem('leadsPage_filters');
      sessionStorage.removeItem('leadsPage_filteredInfo');
    } catch {}
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'ai_segment',
      width: 100,
      filteredValue: filteredInfo.ai_segment || null,
      filters: [
        { text: 'Hot', value: 'Hot' },
        { text: 'Warm', value: 'Warm' },
    ...
    <div>
      <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Space>
          ...
          {canAccessWhatsApp && (
            <Tooltip title="Open WhatsApp Inbox">
              <Button
                icon={<WhatsAppOutlined />}
                onClick={() => setWaInboxVisible(true)}
        ...
        >
          ...
          >
            <Select 
              ...
              notFoundContent={!counselors || counselors.length === 0 ? "Loading counselors..." : "No counselors found"}
              showSearch
              filterOption={(input, option) => 
                option.children.toLowerCase().includes(input.toLowerCase())
              }
      ...
      {/* WhatsApp Inbox — role-aware conversations panel */}
      <WhatsAppInbox
        open={waInboxVisible}
        onClose={() => setWaInboxVisible(false)}
      />
    </div>
  );
};
