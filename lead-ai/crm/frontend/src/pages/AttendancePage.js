...
// Month-wise payroll register (all listed employees) → CSV download.
function downloadPayrollCSV(slips, monthTag) {
  const esc = (v) => {
  ...
  const headers = [
    'Month', 'Employee', 'Email', 'Working Days', 'Present', 'Late', 'Left Early', 'Absent',
    'Paid Festival Days', 'Paid Leaves', 'Gross Salary', 'Incentive',
    'Absent Deduction', 'Late Deduction', 'Advance Deduction', 'Total Deduction',
    'Leave Encashment', 'Net Salary', 'Generated By',
  ];
  const rows = slips.map((s) => [
    dayjs((s.month || '2025-01') + '-01').format('MMMM YYYY'),
    s.user_name, s.user_email,
    num(s.working_days), num(s.days_present), num(s.days_late), num(s.days_left_early), num(s.days_absent),
    num(s.paid_festival_days), num(s.paid_leaves_allowed),
    num(s.gross_salary), num(s.incentive_amount),
    num(s.absent_deduction), num(s.late_deduction_total), num(s.advance_deduction), num(s.total_deduction),
    num(s.leave_payout_amt), num(s.net_salary),
    s.generated_by ? s.generated_by.split('@')[0] : '',
  ]);
  const sum = (k) => slips.reduce((t, s) => t + num(s[k]), 0);
  rows.push([
    'TOTAL', `${slips.length} employee${slips.length !== 1 ? 's' : ''}`, '', '', '', '', '', '',
    '', '', sum('gross_salary'), sum('incentive_amount'),
    sum('absent_deduction'), sum('late_deduction_total'), sum('advance_deduction'), sum('total_deduction'),
    sum('leave_payout_amt'), sum('net_salary'), '',
  ]);
  const csv = [headers, ...rows].map((r) => r.map(esc).join(',')).join('\n');
  const url = URL.createObjectURL(new Blob(['﻿' + csv], { type: 'text/csv;charset=utf-8;' }));
  const a = document.createElement('a');
  a.href = url;
  a.download = `salary_register_${monthTag}.csv`;
  a.click();
          ...
          {isAdmin && slips.length > 0 && (
            <div style={{ marginTop: 20 }}>
              <Button
                ...
                onClick={() => downloadPayrollCSV(slips, filterMonth || 'all-months')}
              >
                Download {filterMonth ? dayjs(filterMonth + '-01').format('MMM YYYY') : 'All'} Payroll CSV
                {' '}({slips.length})
              </Button>
            </div>
          )}
