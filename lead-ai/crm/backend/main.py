...
# Lead enums + all API request/response models now live in schemas.py
# (extracted so routers can import them without importing this whole app).
# schemas.py runs the enum<->constants.py drift assertions at import.
from schemas import (  # noqa: E402  (kept here so LeadStatus is defined before the ORM models below)
    LeadStatus, LeadSegment,
    ...
    CallInitiateRequest, FollowUpRequest, AssignmentRequest, ReassignmentRequest,
    UserCreate, UserUpdate, UserResponse,
    VALID_ROLES, VALID_DEPARTMENTS, VALID_PAGE_KEYS,
)
...
# ============================================================================
# PYDANTIC MODELS (API) — moved to schemas.py, imported near the ORM models above.
# A few request models used by only one endpoint are still defined inline below.
# ============================================================================
...
COUNTRY_DIAL_CODES: dict[str, str] = {
    ...
    'Burundi': '257', 'Cabo Verde': '238', 'Cambodia': '855', 'Cameroon': '237',
    'Canada': '1', 'Central African Republic': '236', 'Chad': '235', 'Chile': '56',
    'China': '86', 'Colombia': '57', 'Comoros': '269', 'Congo (Congo-Brazzaville)': '242',
    'Costa Rica': '506', "Côte d'Ivoire": '225', 'Croatia': '385', 'Cuba': '53',
    'Cyprus': '357', 'Czechia (Czech Republic)': '420',
    'Democratic Republic of the Congo': '243', 'Denmark': '45', 'Djibouti': '253',
    'Dominica': '1767', 'Dominican Republic': '1849',
    'Ecuador': '593', 'Egypt': '20', 'El Salvador': '503', 'Equatorial Guinea': '240',
    'Eritrea': '291', 'Estonia': '372', 'Eswatini (fmr. "Swaziland")': '268',
    ...
    'Haiti': '509', 'Holy See': '379', 'Honduras': '504', 'Hungary': '36',
    ...
    'Mozambique': '258', 'Myanmar (formerly Burma)': '95',
    ...
    'Oman': '968', 'Pakistan': '92', 'Palau': '680', 'Palestine State': '970',
    ...
    'Switzerland': '41', 'Syria': '963', 'Tajikistan': '992', 'Tanzania': '255',
    ...
    'United States of America': '1', 'Uruguay': '598', 'Uzbekistan': '998',
...
def normalize_phone(phone: str, country: str = None) -> str:
    """Return E.164-formatted phone number (+<dialcode><number>).
    - Strips all non-digit characters (except a leading +).
    - If number already starts with +, just clean and return.
    - Otherwise, if country dial code is known and number doesn't already
      start with those digits, prepend the dial code.
    """
    if not phone:
        return phone
    ...
    if dial and not digits.startswith(dial):
...
def normalize_lead_values(lead_data: dict) -> dict:
    ...
    # Covers all 195 UN-recognized countries plus Indian states → India
    country_map = {
        # India
        'in': 'India', 'ind': 'India', 'india': 'India',
        # Indian states & cities → India
        'maharashtra': 'India', 'maharastra': 'India', 'bihar': 'India',
        'karnataka': 'India', 'uttar pradesh': 'India', 'up': 'India',
        'west bengal': 'India', 'punjab': 'India', 'madhya pradesh': 'India',
        'mp': 'India', 'tamilnadu': 'India', 'tamil nadu': 'India',
        'kerala': 'India', 'keralam': 'India', 'gujarat': 'India', 'gujrat': 'India',
        'andhra pradesh': 'India', 'assam': 'India', 'haryana': 'India',
        'jharkhand': 'India', 'rajasthan': 'India', 'telangana': 'India',
        'telengana': 'India', 'uttrakhand': 'India', 'himachal pradesh': 'India',
        'chhattisgarh': 'India', 'jammu and kashmir': 'India', 'j&k': 'India',
        'h.p': 'India', 'mh': 'India', 'as': 'India',
        # ISO-2 → full name
        'af': 'Afghanistan',
        'al': 'Albania',
        'dz': 'Algeria',
    ...
    # Normalize phone & whatsapp → E.164
    _country_for_phone = normalized.get('country', '')
    if normalized.get('phone'):
        normalized['phone'] = normalize_phone(normalized['phone'], _country_for_phone)
    if normalized.get('whatsapp'):
        normalized['whatsapp'] = normalize_phone(normalized['whatsapp'], _country_for_phone)
...
def _fetch_all_leads(columns: str, filters: dict = None, date_from: str = None,
    ...
    page_size = 1000
    while True:
        q = supabase_data.client.table('leads').select(columns)
        if filters:
...
async def get_dashboard_stats(request: Request, created_from: Optional[str] = None, created_to: Optional[str] = None):
    ...
    try:
        ...
        # Get avg score — paginate to bypass Supabase 1000-row default
        scores = []
        _sc_offset = 0
        while True:
...
async def assign_all_unassigned(
    ...
    # Get unassigned leads (paginated)
    unassigned = []
    _ua_offset = 0
    while True:
        _ua_resp = supabase_data.client.table('leads').select('lead_id').is_('assigned_to', 'null').range(_ua_offset, _ua_offset + 999).execute()
        _ua_batch = _ua_resp.data or []
        unassigned.extend(_ua_batch)
        if len(_ua_batch) < 1000:
            break
        _ua_offset += 1000
...
async def trigger_workflows():
    ...
    try:
        # Get all leads
        all_leads = _fetch_all_leads('lead_id,status,follow_up_date')
...
async def send_communication(
    ...
    if comm_type not in {"whatsapp", "email"}:
        raise HTTPException(status_code=400, detail="Unsupported communication type")
    ...
    result = await comm_service.send(
        ...
        subject=payload.subject or "Message from Medical CRM",
        media_url=payload.media_url,
        media_type=payload.msg_type if payload.msg_type != "text" else None
    )
    ...
    history_payload = {
        ...
        "communication_metadata": {
            "provider": result.get("provider", comm_type),
    ...
    supabase_data.create_communication_history(history_payload)
...
async def initiate_communication_call(
    ...
    result = {
        ...
        "call_sid": call_sid,
        "status": "simulated",
        "message": "Call initiated in simulated mode."
    }
    ...
    supabase_data.create_communication_history({
        ...
        "direction": "outbound",
        "content": f"Call initiated to {payload.to_number}",
        "timestamp": datetime.utcnow().isoformat(),
        "status": "initiated",
        "communication_metadata": {
            "provider": "simulated",
...
async def mark_communications_training(
    ...
    if not ids:
        raise HTTPException(status_code=400, detail="id or ids list is required")
    if not supabase_data.mark_training_data(ids):
        raise HTTPException(status_code=500, detail="Failed to update training status")
    return {"success": True, "marked_count": len(ids)}
...
async def get_admin_stats():
    ...
    try:
        ...
        _offset = 0
        while True:
            _resp = supabase_data.client.table('leads').select('status,ai_segment,expected_revenue,created_at').range(_offset, _offset + 999).execute()
...
async def get_user_performance(user_id: int, days: int = 7,
    ...
    try:
        ...
        leads = _fetch_all_leads('status,created_at', {'assigned_to': user['full_name']})
...
# ============================================================================
# META WHATSAPP CLOUD API — DIRECT INTEGRATION WITH RBAC
# ============================================================================

_META_WA_TOKEN = os.getenv("META_WHATSAPP_ACCESS_TOKEN", "")
_META_WA_PHONE_ID = os.getenv("META_WHATSAPP_PHONE_NUMBER_ID", "")
_META_WA_VERIFY_TOKEN = os.getenv("META_WHATSAPP_WEBHOOK_VERIFY_TOKEN", "crm_verify_token")
...
def _meta_wa_send_text(to_number: str, message: str) -> dict:
    ...
    try:
        r = _req.post(
            ...
            json=payload, headers=_meta_wa_headers(), timeout=10,
        )
        data = r.json()
        if r.status_code == 200:
            msg_id = data.get("messages", [{}])[0].get("id", "")
            return {"success": True, "message_id": msg_id}
        return {"success": False, "error": data.get("error", {}).get("message", "Send failed")}
    except Exception as e:
        return {"success": False, "error": str(e)}


def _meta_wa_mark_read(message_id: str):
    import requests as _req
    try:
        _req.post(
            f"{_META_WA_API}/{_META_WA_PHONE_ID}/messages",
...
def _get_lead_by_phone(phone: str) -> Optional[dict]:
    """Find a lead by last-10-digits match on whatsapp or phone field."""
    normalized = re.sub(r"[^\d]", "", phone)
    last10 = normalized[-10:] if len(normalized) >= 10 else normalized
    try:
        r = supabase_data.client.table("leads").select(
            "id,lead_id,full_name,whatsapp,phone,assigned_to"
        ).ilike("whatsapp", f"%{last10}%").limit(1).execute()
        if r.data:
            return r.data[0]
        r = supabase_data.client.table("leads").select(
            "id,lead_id,full_name,whatsapp,phone,assigned_to"
...
async def meta_whatsapp_webhook(request: FARequest):
    """
        ...
        for entry in body.get("entry", []):
            for change in entry.get("changes", []):
                ...
                for msg in value.get("messages", []):
                    ...
                    lead = _get_lead_by_phone(from_number)
                    assigned_agent = lead.get("assigned_to") if lead else None

                    ts_iso = (
                        datetime.utcfromtimestamp(int(timestamp)).isoformat()
                    ...
                    supabase_data.client.table("whatsapp_messages").insert({
                        ...
                        "to_number": _META_WA_BUSINESS_NUMBER,
                    ...
                    _meta_wa_mark_read(message_id)
                    logger.info(f"Meta WA inbound {from_number}: {content[:60]}")
...
async def meta_whatsapp_send(
    ...
    return {"success": True, "message_id": result.get("message_id"), "to": phone}
...
async def get_whatsapp_conversations(
    ...
    try:
        ...
        logger.error(f"Error fetching WA conversations: {e}")
        return []
...
async def get_whatsapp_history(
    ...
    try:
        ...
        logger.error(f"Error fetching WA history for {phone_number}: {e}")
        return []
...
async def mark_whatsapp_read(
    ...
    try:
        ...
        supabase_data.client.table("whatsapp_conversations").update(
            {"unread_count": 0}
        ).eq("phone_number", phone_number).execute()

        return {"success": True}
    except Exception as e:
        logger.error(f"Error marking WA read for {phone_number}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
